...
class SegmentedWindArrow(Widget):
    ...
    def __init__(self, cx=120, cy=120, radius=120):
        ...
        # Tweak width/height if you change arrow geometry.
        self.wipe_local = [
            (-10, -18),
            ( 10, -18),
            ( 10,  38),
        ...
        # Draw wipe first inside this widget so arrow is on top of it.
        ...
        # Triangle tip pointing "up" in local coords; rotation aims it outward
        ...
        for i in range(4):
            ...
            # y is behind the head (positive y is "down" in local coords)
        ...
        # initial
        self.set(wind_dir_deg=0, wind_speed_kts=0)

    # ---- helpers ----

    def _rotate_points(self, pts, c, s):
        # Rotate around origin (0,0) in local coords
        out = []
        for x, y in pts:
    ...
    def _speed_to_blocks(self, kts):
        # 0 blocks below 1kt; then 1 per band
        if kts < 1:
            return 0
        elif kts < 5:
            return 1
    ...
    def set(self, wind_dir_deg, wind_speed_kts):
        ...
        # Light up blocks
        n = self._speed_to_blocks(float(wind_speed_kts))
        for i, (_, pal) in enumerate(self.blocks):
            pal[0] = self.block_colors[i] if i < n else self.block_off_color
